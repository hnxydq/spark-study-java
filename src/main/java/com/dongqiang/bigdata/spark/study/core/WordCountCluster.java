...
public class WordCountCluster {
	public static void main(String[] args) {
		...
		JavaRDD<String> lines = sc.textFile("hdfs://spark1.dongqiang.com:8020");
		//java 7 style
		/*JavaRDD<String> lineRdd = lines.flatMap(new FlatMapFunction<String, String>() {
		...
		wordPairRdd.foreach(new VoidFunction<Tuple2<String,Integer>>() {
			...
			public void call(Tuple2<String, Integer> t) throws Exception {
				System.out.println("word: " + t._1 + " appears " + t._2 + " times.");
			}
		});*/

		//java8 style
		JavaRDD<String> lineRdd = lines.flatMap(line -> Arrays.asList(line.split(" ")));
		JavaPairRDD<String, Integer> wordPairRdd = lineRdd.mapToPair(line -> new Tuple2<>(line, 1));
		JavaPairRDD<String, Integer> wordCount = wordPairRdd.reduceByKey((count1, count2) -> count1 + count2);
		wordCount.foreach(item -> System.out.println("word: " + item._1 + " appears " + item._2 + " times."));
		sc.close();
	}
}
